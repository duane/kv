...
func TestFlush(t *testing.T) {
  test_data := HashMemTable(map[string][]byte{
  ...
  err := test_data.Flush("test.out")
  if err != nil {
    t.Fatalf("%+v", err)
  }

  test_data2 := HashMemTable(map[string][]byte{})
  err = test_data2.Read("test.out")
  if err != nil {
    t.Fatalf("%+v", err)
  }

  for k, v := range test_data {
    v2, err := test_data2.Get(k)
    if err != nil {
      panic(err.Error())
    }
  ...
  for k, v2 := range test_data2 {
    v, err := test_data.Get(k)
    if err != nil {
      panic(err.Error())
    }
    if bytes.Compare(v, v2) != 0 {
